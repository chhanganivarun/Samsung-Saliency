...
def make_dataset(annotation_path, audio_path, gt_path, vox=False, json_file=None):
    ...
    for i in range(len(video_names)):
        ...
        if not os.path.exists(audio_wav_path):
            ...
            continue
        if not vox:
            [audiowav, Fs] = torchaudio.load(
                audio_wav_path, normalization=False)
...
def validate(args):
    ...
    with open(join(args.path_indata, 'fold_lists', file_name), 'r') as f:
    ...
    for dname in list_indata:
        ...
        list_frames = [f for f in os.listdir(os.path.join(path_indata, 'video_frames', args.dataset, dname)) if os.path.isfile(
            os.path.join(path_indata, 'video_frames', args.dataset, dname, f))]
        list_frames.sort()
        ...
        if len(list_frames) >= 2*len_temporal-1:
            ...
            for i in range(len(list_frames)):
                torch_img, img_size = torch_transform(os.path.join(
                    path_indata, 'video_frames', args.dataset, dname, list_frames[i]))
                ...
                if i >= len_temporal-1:
                    ...
                    if args.use_sound:
                        audio_feature = get_audio_feature(
                            dname, audiodata, args, i-len_temporal+1)
                    process(model, clip, path_indata, dname,
                            list_frames[i], args, img_size, audio_feature=audio_feature)

                    if i < 2*len_temporal-2:
                        if args.use_sound:
                            audio_feature = torch.flip(audio_feature, [2])
                        process(model, torch.flip(clip, [
                                2]), path_indata, dname, list_frames[i-len_temporal+1], args, img_size, audio_feature=audio_feature)
...
def process(model, clip, path_inpdata, dname, frame_no, args, img_size, audio_feature=None):
    with torch.no_grad():
        if audio_feature is None:
            ...
            smap = model(clip.to(device), audio_feature.to(
                device)).cpu().data[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--file_weight', default="./saved_models/AViNet_Dave.pt", type=str)
    parser.add_argument('--nhead', default=4, type=int)
    parser.add_argument('--num_encoder_layers', default=3, type=int)
    parser.add_argument('--transformer_in_channel', default=512, type=int)
    parser.add_argument(
        '--save_path', default='/ssd_scratch/cvit/varunc/Results/diem_test', type=str)
