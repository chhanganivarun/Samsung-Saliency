...
pretrained_settings = {
    'bninception': {
        ...
        'kinetics': {
            ...
            'input_size': 224,
            'input_range': [0, 255],
            # [96.29023126, 103.16065604, 110.63666788]
            'mean': [104, 117, 128],
...
class BNInception(nn.Module):
    ...
    def features(self, x):
        ...
        # stage2
        pool2_3x3_s2_out = self._temporal_forward_wrap(
            self._block_2, 1)(pool1_3x3_s2_out)

        # stage3
        inception_3a_output_out = self._temporal_forward_wrap(
            self._block_3a, 2)(pool2_3x3_s2_out)
        inception_3b_output_out = self._temporal_forward_wrap(
            self._block_3b, 3)(inception_3a_output_out)
        inception_3c_output_out = self._temporal_forward_wrap(
            self._block_3c, 4)(inception_3b_output_out)

        inception_4a_output_out = self._temporal_forward_wrap(
            self._block_4a, 5)(inception_3c_output_out)
        inception_4b_output_out = self._temporal_forward_wrap(
            self._block_4b, 6)(inception_4a_output_out)
        inception_4c_output_out = self._temporal_forward_wrap(
            self._block_4c, 7)(inception_4b_output_out)
        inception_4d_output_out = self._temporal_forward_wrap(
            self._block_4d, 8)(inception_4c_output_out)
        inception_4e_output_out = self._temporal_forward_wrap(
            self._block_4e, 9)(inception_4d_output_out)

        inception_5a_output_out = self._temporal_forward_wrap(
            self._block_5a, 10)(inception_4e_output_out)
        inception_5b_output_out = self._temporal_forward_wrap(
            self._block_5b, 11)(inception_5a_output_out)
    ...
    def _temporal_forward_wrap(self, layer_func, index):
        ...
        if hasattr(self, 'is_temporal_shift') and self.is_temporal_shift[index] == '1':
            ...
            def wrapped_func(x, is_residual, n_segment, fold_div):
                if is_residual:
                    x_shift = TemporalShift.shift(
                        x, n_segment, fold_div=fold_div)
    ...
    def _build_features(self, inplace, num_classes):
        ...
        self.inception_5b_pool = nn.MaxPool2d((3, 3), stride=(
            1, 1), padding=(1, 1), dilation=(1, 1), ceil_mode=True)
