...
from tsm.ops.basic_ops import ConsensusModule
from tsm.ops.transforms import *
from torch.nn.init import normal_, constant_


class TSN(nn.Module):
    def __init__(self, num_segments=8, modality='RGB',
    ...
    def _prepare_tsn(self):
        ...
        from model import DecoderConv2D, DecoderConv2D3hier
        from model import Identity, Reshape
        # if self.dropout == 0:
        # setattr(self.base_model, self.base_model.last_layer_name,
        #         Identity())
        # self.new_decoder = DecoderConvUpNoHier('tsm')
        # else:
        ...
        if 'resnet' in self.base_model_name:
            ...
            channel = 1280
        setattr(self.base_model, self.base_model.last_layer_name,
                Reshape((channel, 7, 12)))
        # setattr(self.base_model, self.base_model.last_layer_name,
        #         Identity())
        if 'resnet' in self.base_model_name:
            self.new_decoder = DecoderConv2D(self.base_model_name)
        else:
            self.new_decoder = DecoderConv2D3hier(self.base_model_name)

        # std = 0.001
        # if self.new_decoder is None:
        ...
        # else:
        #     if hasattr(self.new_decoder, 'weight'):
        #         normal_(self.new_decoder.weight, 0, std)
        #         constant_(self.new_decoder.bias, 0)
        # return feature_dim
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            from tsm.archs.mobilenet_v2 import mobilenet_v2, InvertedResidual
            ...
            from tsm.archs.bn_inception import bninception
    ...
    def forward(self, input, no_reshape=False):
        if not no_reshape:
            ...
            in_vec = input.contiguous().view(
                (-1, sample_len) + input.size()[-2:])

            base_out = self.base_model(in_vec)
        else:
        ...
        if 'resnet' in self.base_model_name:
            base_out = base_out.view(
                (input.size()[0], -1)+base_out.size()[1:])
            base_out = base_out.sum(axis=1)

            base_out = self.new_decoder(base_out)
        else:
            base_out = [y.view(
                (input.size()[0], -1)+y.size()[1:]) for y in base_out]
            base_out = [y.sum(axis=1) for y in base_out]
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        # nn.modules.children() return all sub modules in a DFS manner
        modules = list(self.base_model.modules())
        first_conv_idx = filter(lambda x: isinstance(
            modules[x], nn.Conv2d), list(range(len(modules))))[0]
        conv_layer = modules[first_conv_idx]
        container = modules[first_conv_idx - 1]
