...
def normalize_map(s_map):
    ...
    min_s_map = torch.min(s_map.view(batch_size, -1),
                          1)[0].view(batch_size, 1, 1).expand(batch_size, w, h)
    max_s_map = torch.max(s_map.view(batch_size, -1),
                          1)[0].view(batch_size, 1, 1).expand(batch_size, w, h)
...
def cc(s_map, gt):
    ...
    mean_s_map = torch.mean(s_map.view(batch_size, -1),
                            1).view(batch_size, 1, 1).expand(batch_size, w, h)
    std_s_map = torch.std(s_map.view(batch_size, -1),
                          1).view(batch_size, 1, 1).expand(batch_size, w, h)

    mean_gt = torch.mean(gt.view(batch_size, -1),
                         1).view(batch_size, 1, 1).expand(batch_size, w, h)
    std_gt = torch.std(gt.view(batch_size, -1),
                       1).view(batch_size, 1, 1).expand(batch_size, w, h)
...
def nss(s_map, gt):
    if s_map.size() != gt.size():
        ...
        s_map = torch.FloatTensor(cv2.resize(
            s_map, (gt.size(2), gt.size(1)))).unsqueeze(0)
        s_map = s_map.cuda()
        gt = gt.cuda()
...
def auc_judd(saliencyMap, fixationMap, jitter=True, toPlot=False, normalize=False):
    ...
    if saliencyMap.size() != fixationMap.size():
        ...
        saliencyMap = torch.FloatTensor(cv2.resize(
            saliencyMap, (fixationMap.size(2), fixationMap.size(1)))).unsqueeze(0)
...
def auc_shuff(s_map, gt, other_map, splits=100, stepsize=0.1):
    ...
    for i in random_numbers:
        ...
        for k in i:
            r_sal_map.append(
                s_map[k % s_map.shape[0]-1, int(k/s_map.shape[0])])
