...
def make_dataset(annotation_path, audio_path, gt_path, json_file=None):
    ...
    for i in range(len(video_names)):
        ...
        for videoframe in range(1, n_frames+1):
            startemp = max(
                0, ((videoframe-1)*(1.0/float(video_fps[i]))*Fs)-n_samples/2)
            starts[videoframe] = int(startemp)
            endtemp = min(audiowav.shape[1], abs(
                ((videoframe-1)*(1.0/float(video_fps[i]))*Fs)+n_samples/2))
...
def get_audio_feature(audioind, audiodata, clip_size, start_idx):
    ...
    if audioind in audiodata:
        ...
        try:
            valid['audio'] = audiodata[audioind]['wav'][:,
                                                        excerptstart:excerptend+1].shape[1]
        except:
            pass
        audioexcer_tmp = audiodata[audioind]['wav'][:,
                                                    excerptstart:excerptend+1]
        if (valid['audio'] % 2) == 0:
            audioexcer[:, ((audioexcer.shape[1]//2)-(valid['audio']//2)):((audioexcer.shape[1]//2)+(valid['audio']//2))] = \
                torch.from_numpy(np.hanning(
                    audioexcer_tmp.shape[1])).float() * audioexcer_tmp
        else:
...
class SoundDatasetLoader(Dataset):
    def __init__(self, len_snippet, dataset_name='DIEM', split=1, mode='train', use_sound=False, use_vox=False):
        ''' mode: train, val, save '''
        path_data = '/ssd_scratch/cvit/varunc/vinet/'
        ...
        if dataset_name == 'DIEM':
            ...
            file_name = '{}_list_{}_{}_fps.txt'.format(
                dataset_name, mode, split)
        ...
        if self.mode == 'train':
            self.list_num_frame = [len(os.listdir(os.path.join(
                path_data, 'annotations', dataset_name, v, 'maps'))) for v in self.list_indata]

        elif self.mode == 'test' or self.mode == 'val':
            print("val set")
            for v in self.list_indata:
                frames = os.listdir(
                    join(path_data, 'annotations', dataset_name, v, 'maps'))
                frames.sort()
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            ...
            while 1:
                start_idx = np.random.randint(
                    0, self.list_num_frame[idx]-self.len_snippet+1)
        ...
        path_clip = os.path.join(
            self.path_data, 'video_frames', self.dataset_name, video_name)
        path_annt = os.path.join(
            self.path_data, 'annotations', self.dataset_name, video_name, 'maps')

        if self.use_sound:
            audio_feature = get_audio_feature(
                video_name, self.audiodata, self.len_snippet, start_idx)

        clip_img = []

        for i in range(self.len_snippet):
            img = Image.open(join(path_clip, 'img_%05d.jpg' %
                             (start_idx+i+1))).convert('RGB')
            sz = img.size
            clip_img.append(self.img_transform(img))

        clip_img = torch.FloatTensor(torch.stack(clip_img, dim=0))

        gt = np.array(Image.open(join(path_annt, 'eyeMap_%05d.jpg' %
                      (start_idx+self.len_snippet))).convert('L'))
        gt = gt.astype('float')
...
class DHF1KDataset(Dataset):
    def __init__(self, path_data, len_snippet, mode="train", multi_frame=0, alternate=1):
        ...
        if self.mode == "train":
            ...
            self.list_num_frame = [
                len(os.listdir(os.path.join(path_data, d, 'images'))) for d in self.video_names]
        elif self.mode == "val":
            self.list_num_frame = []
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.mode == "train":
                ...
                start_idx = np.random.randint(
                    0, self.list_num_frame[idx]-self.alternate * self.len_snippet+1)
            ...
            for i in range(self.len_snippet):
                img = Image.open(os.path.join(path_clip, '%04d.png' %
                                              (start_idx+self.alternate*i+1))).convert('RGB')
                sz = img.size

                if self.mode != "save":
                    gt = np.array(Image.open(os.path.join(path_annt, '%04d.png' % (
                        start_idx+self.alternate*i+1))).convert('L'))
                    gt = gt.astype('float')
            ...
            print(sorted(self.video_names))
            print(idx, file_name)
            print(self.list_num_frame[idx], self.alternate, self.len_snippet)
            print(self.list_num_frame[idx]-self.alternate * self.len_snippet+1)
...
class Hollywood_UCFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.mode == "train":
            file_name = self.video_names[idx]
            start_idx = np.random.randint(
                0, max(1, self.list_num_frame[idx]-self.len_snippet+1))
        elif self.mode == "val":
            (file_name, start_idx) = self.list_num_frame[idx]
        ...
        if len(list_sal_clips) < self.len_snippet:
            temp = [list_clips[0]
                    for _ in range(self.len_snippet-len(list_clips))]
            temp.extend(list_clips)
            list_clips = copy.deepcopy(temp)

            temp = [list_sal_clips[0]
                    for _ in range(self.len_snippet-len(list_sal_clips))]
            temp.extend(list_sal_clips)
            list_sal_clips = copy.deepcopy(temp)

            assert len(list_sal_clips) == self.len_snippet and len(
                list_clips) == self.len_snippet

        for i in range(self.len_snippet):
            img = Image.open(os.path.join(
                path_clip, list_clips[start_idx+i])).convert('RGB')
            clip_img.append(self.img_transform(img))

            gt = np.array(Image.open(os.path.join(
                path_annt, list_sal_clips[start_idx+i])).convert('L'))
            gt = gt.astype('float')
...
def get_audio_feature_vox(audioind, audiodata, clip_size, start_idx):
    ...
    if audioind in audiodata:
        ...
        # except:
        # 	pass
        audio_feature_tmp = audiodata[audioind]['wav'][:,
                                                       excerptstart:excerptend+1]
